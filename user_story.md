...
Goal: Allow the author to set up the project and manage the manuscript flow.
...
AC 1: Metadata table in SQLite stores book_title and total_chapters.
...
AC 1: Back-office provides a text area for Markdown input.

AC 2: Content is saved in the chapters table in SQLite.
...
So that I don't have to manage another password but my comments are still attributed to me.

AC 1: Entry screen blocks access until a "Pseudo" is provided.

AC 2: Pseudo is stored in sessionStorage or a cookie.
...
Goal: Enable contextual feedback on web and mobile.
...
AC 2: Selection offsets (start/end) are correctly captured and saved in SQLite.
...
AC 1: Touch start/end detection triggers the annotation modal after 500ms if text is selected.

AC 2: Prevents the native mobile menu from obstructing the custom UI.
...
AC 2: Replies are stored with a parent_id in the annotations table.
...
AC 1: Admin can toggle status from "Awaiting Feedback" to "Validated".

AC 2: Moving to "Validated" locks the chapter; no new annotations can be added.